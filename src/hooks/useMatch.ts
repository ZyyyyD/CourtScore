...
export async function joinMatch(
  ...
  if (isGuest) {
    return {
      error: "Sign in to join a friend's match. Guest mode is local-only.",
    };
  }
  ...
  if (matchError)
    console.error("[joinMatch] match lookup error:", matchError.message);
  ...
  // Find the next open position for team_b to avoid the unique constraint
  // on (match_id, team_side, position).
  ...
  const maxSlots = match.match_type === "doubles" ? 2 : 1;
  const taken = new Set((teamBRows ?? []).map((r: { position: number }) => r.position));
  const nextPosition = ([1, 2] as const).find((p) => !taken.has(p));

  if (!nextPosition || taken.size >= maxSlots) {
    return { error: "This match is full." };
  }
