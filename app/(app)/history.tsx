...
export default function HistoryScreen() {
  ...
  const FilterBtn = ({ f, label }: { f: Filter; label: string }) => (
    <Pressable onPress={() => setFilter(f)} style={[styles.filterBtn, filter === f && styles.filterActive]}>
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.cardLeft}>
              ...
              <Text style={styles.type}>{item.match_type === 'singles' ? 'Singles' : 'Doubles'} · To {item.scoring_target}</Text>
            </View>
            <View style={styles.cardRight}>
              <Text style={styles.score}>{item.score_a} – {item.score_b}</Text>
              <Badge
                label={item.winner_side === item.match_players.find((p) => p.player_id === user?.id)?.team_side ? 'WIN' : 'LOSS'}
                variant={item.winner_side === item.match_players.find((p) => p.player_id === user?.id)?.team_side ? 'win' : 'loss'}
              />
            </View>
          </Pressable>
        )}
...
const styles = StyleSheet.create({
  header: { padding: Spacing.lg, gap: Spacing.md, borderBottomWidth: 1, borderBottomColor: Colors.outlineVariant },
  ...
  filterActive: { backgroundColor: Colors.primaryContainer, borderColor: Colors.primary },
